...
/* --------------------------------------------------------------------------
 * SHA-1 streaming API via BearSSL
 *
 * sha1Init()              → opaque handle
 * sha1Update(handle, data) — feed string or TypedArray
 * sha1Out(handle)         → ArrayBuffer(20), does not consume context
 * -------------------------------------------------------------------------- */
...
static JSValue js_tls_sha1Init(JSContext *ctx, JSValueConst this_val,
                               int argc, JSValueConst *argv) {
...
static JSValue js_tls_sha1Update(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv) {
...
static JSValue js_tls_sha1Out(JSContext *ctx, JSValueConst this_val,
                              int argc, JSValueConst *argv) {
	br_sha1_context *sc = JS_GetOpaque2(ctx, argv[0], sha1_class_id);
	if (!sc) return JS_EXCEPTION;
	...
	return JS_NewArrayBufferCopy(ctx, out, br_sha1_SIZE);
}
...
static const JSCFunctionListEntry js_tls_funcs[] = {
	...
	JS_CFUNC_DEF("sha1Init", 0, js_tls_sha1Init),
	JS_CFUNC_DEF("sha1Update", 2, js_tls_sha1Update),
	JS_CFUNC_DEF("sha1Out", 1, js_tls_sha1Out),
};
