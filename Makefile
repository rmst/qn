...
$(QNODE_PROG): qnode/bootstrap.js qnode/node-globals.js qnode/node/* qnode/repl.js qx/index.js qx/core.js $(QJSXC_PROG) quickjs-deps | $(BIN_DIR)
	QJSXPATH=./qnode:./qx $(QJSXC_PROG) -D node-globals -D repl -D node:fs -D node:process -D node:child_process -D node:crypto -D node:path -D node:events -D node:stream -D node:buffer -D node:url -D qx -o $@ qnode/bootstrap.js

# Build qx (zx-compatible shell scripting with $ function)
$(QX_PROG): qx/bootstrap.js qnode/node-globals.js qx/* qnode/node/* qnode/repl.js $(QJSXC_PROG) quickjs-deps | $(BIN_DIR)
	QJSXPATH=./qnode:./qx $(QJSXC_PROG) -D node-globals -D repl -D node:fs -D node:process -D node:child_process -D node:crypto -D node:path -D node:events -D node:stream -D node:buffer -D node:url -D qx/core -o $@ qx/bootstrap.js
